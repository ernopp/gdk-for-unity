...
The FPS starter project contains a health pack prefab, `HealthPack.prefab`, in the folder: `/Assets/Fps/Resources/Prefabs/`.

In the `/Assets/Fps/Resources/Prefabs/UnityClient/` folder create a new prefab named **"HealthPickup"**. Give this two children, the health pack and the base plate (`/Assets/Fps/Resources/Prefabs/HealthPackCube.prefab` and `/Assets/Fps/Resources/Prefabs/HealthPackBase.prefab`).
...
Similar to how we created a client-side prefab, navigate to the `/Assets/Fps/Resources/Prefabs/UnityGameLogic/` folder and create another `HealthPickup` prefab. Once again this name must match the `Metadata` string for the health pack entity so that the GameObject Creation system knows to associate them.

Add a collider to the newly created `HealthPickup` prefab, so that our server-side workers can trigger collisions with our pickups.
